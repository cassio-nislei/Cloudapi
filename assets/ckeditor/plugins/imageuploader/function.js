...
function checkUpload(){
    if( document.getElementById("upload").files.length == 0 ){
        alert("Nenhum arquivo selecionado para ser enviado.");
        return false;
    }
}
...
function reloadImages() {
    ...
    $('#files').load('function.php?f=loadImages', function(response, status, xhr) {
     ...
     $('#files2').load('function.php?f=loadFiles', function(response, status, xhr) {
         $("img.lazy").lazyload();
     });*/
    //$.post('function.php', { f: 'loadImages' }, function(data) { $('#files').html(data); $("img.lazy").lazyload();}, 'html');
    //$.post('function.php', { f: 'loadFiles'  }, function(data) { $('#files2').html(data); $("img.lazy").lazyload();}, 'html');
}
