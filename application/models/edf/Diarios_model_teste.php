...
class Diarios_model extends CI_Model {
    ...
    private $tabela = 'edf_diarios';
    private $view = 'vw_edf_diarios';
    
    function __construct() {
        parent::__construct();
    }
    
    public function get($id, $modelo = null, $view = FALSE) {
        $this->db->where('id', $id);
        $this->db->where(['emitente_id' => (int)$this->session->userdata('emit.id'), 
                          'filial_id'   => $this->session->userdata('user.filial_id')]);
        $g = $this->db->get($view ? $this->view : $this->tabela);
    ...
    function isTemp($id) {
        return $this->db->where(['id' => (int)$id, 'status' => 'T'])
                        ->count_all_results($this->tabela) > 0;
    }
    
    function isConfirmado($id) {
        return $this->db->where(['id' => (int)$id, 'status' => 'C'])
                        ->count_all_results($this->tabela) > 0;
    }
