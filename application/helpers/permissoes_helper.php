...
function isAdmin() {
     ...
     * Tesoureiros = 46
     */
    //return get_grupo_id() === 1;
    return get_grupo_nome() === 'administradores';
}

function isSysOp() {
   $CI =& get_instance();
   return (int)$CI->session->userdata('user.sysop') === TRUE;
}

function isPastor() {
    //return get_grupo_id() === 45;  
    return get_grupo_nome() === 'pastores';
}

function isTesoureiro() {
    //return get_grupo_id() === 46;
    return get_grupo_nome() === 'tesoureiros';
}

function get_grupo_id() {
    $CI =& get_instance();
    return (int)$CI->session->userdata('user.grupo_id'); 
}

function get_grupo_nome() {
    $CI =& get_instance();
    return is5_strtolower($CI->session->userdata('user.grupo')); 
}

function get_user_id() {
    $CI =& get_instance();
    return (int)$CI->session->userdata('user.ID');
}

function get_emit_id() {
    $CI =& get_instance();
    return (int)$CI->session->userdata('emit.id');
}
